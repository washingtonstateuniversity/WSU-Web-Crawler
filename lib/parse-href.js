...
ParseHref.prototype.get_url = function get_url( href, source_uri ) {
	...
	if ( "robosub.eecs.wsu.edu" === url.hostname && url.path.indexOf( "/wiki/" ) === 0 ) {
		return false;
	}

	// There are a lot of combined tags that create many URL paths.
	if ( "www.tfrec.wsu.edu" === url.hostname && url.path.indexOf( "/pages/orgrte/browseD/" ) === 0 ) {
		return false;
	}

	// This is a very strange calendar.
	if ( "cw.wsu.edu" === url.hostname && url.path.indexOf( "/calendar" ) === 0 ) {
		return false;
	}

	// The calendar used at this domain generates a bunch of URLs.
	if ( "multicorereu.eecs.wsu.edu" === url.hostname && url.path.indexOf( "/calendar/" ) === 0 ) {
		return false;
	}

	// Everything on libraries.wsu.edu has a print link.
	if ( "libraries.wsu.edu" === url.hostname && url.path.indexOf( "/print/" ) === 0 ) {
		return false;
	}

	// There is a deep web of auto generated maps.
	if ( "libraries.wsu.edu" === url.hostname && url.path.indexOf( "/map.php" ) === 0 ) {
		return false;
	}

	// A lot of dynamic filters here.
	if ( "libraries.wsu.edu" === url.hostname && url.path.indexOf( "/databases/browse" ) === 0 ) {
		return false;
	}

	// This accounts for over 2 million URLs that are effectively the same.
	if ( url.path.indexOf( "tribe_paged" ) >= 0 || url.path.indexOf( "tribe_venue" ) >= 0 || url.path.indexOf( "tribe_organizer" ) >= 0  || url.path.indexOf( "post_type=tribe_events" ) >= 0 ) {
		return false;
	}

	// Ignore The Event Calendar ICS download URLs.
	if ( url.path.indexOf( "?ical=1" ) >= 0 ) {
		return false;
	}

	// This calendar has no bounds - hello February, 1830!
	if ( url.path.indexOf( "jtCalendar.php?" ) >= 0 ) {
		return false;
	}
	...
	if ( "research.libraries.wsu.edu" === url.hostname ) {
		if ( url.path.indexOf( "/feed/" ) >= 0 || url.path.indexOf( "search-filter?" ) >= 0 || url.path.indexOf( "discover?" ) >= 0 ) {
			return false;
		}
	}

	if ( "thebreadlab.wsu.edu" === url.hostname && url.path.indexOf( "/2017-grain-gathering-schedule/action~" ) === 0 ) {
		return false;
	}
	...
	if ( "wsm.wsu.edu" === url.hostname && "" !== url.query && null !== url.query ) {
	...
	if ( "alumni.wsu.edu" === url.hostname && "" !== url.query && null !== url.query ) {
		let parts = url.query.split( "&" );
		let new_parts = [];
		for ( let j = 0; j < parts.length; j++ ) {
			if ( 0 === parts[ j ].search( /(sid|gid)/ ) ) {
				continue;
			}
	...
	if ( ( "digitalexhibits.libraries.wsu.edu" === url.hostname || "irishinitiative.libraries.wsu.edu" === url.hostname ) && "" !== url.query && null !== url.query ) {
		let parts = url.query.split( "&" );
		let new_parts = [];
		for ( let j = 0; j < parts.length; j++ ) {
	...
	if ( "connections.store.wsu.edu" === url.hostname || "oc.store.wsu.edu" === url.hostname ) {
		if ( url.path.indexOf( "/catalog/product_compare" ) === 0 || url.path.indexOf( "/wishlist/" ) === 0 ) {
			return false;
		}
	}
